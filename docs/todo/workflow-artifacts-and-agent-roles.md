...
>
> 状态：薄 Harness、成功配对运行和可选主程序接入已完成；待人工 A/B 与异题复验
...
这仍不证明 roles 的作品更好，也不能用单次调用数和耗时证明稳定成本优势。主程序默认继续使用 single-agent，
用户可通过 `/agent composition-ab` 显式启用角色模式。
...
8. 主程序使用持久化 Agent 模式路由；角色结果进入同一个工作候选、试听和显式接受闭环。

## 已验证的不变量

- 角色只产出候选内容；Project 仍独占工作乐谱、候选接受和版本事实。
...
- 综合多次运行的技术稳定性、调用成本和人工听感，决定继续保留或撤回可选角色模式。
