...
现有主程序默认复用 single-agent、`inspect_alda_source`、候选检查点、工作乐谱和 MIDI/WAV 链路，不引入分支
乐谱、通用工作流引擎或自动审美评分。原生 Alda 加薄 Harness 的角色路径已作为显式可选
`composition-ab` 模式接入；下一阶段仍是完整人工 A/B 和不同曲式或拍号的异题复验，不把它扩展为通用
Multi-Agent 平台。
...
实验纵切与角色 A/B 已能报告并拒绝以下问题；主程序可选模式继续保留这些边界：
