...
>
> 状态：第二次代表性长曲已完成；结构机制有效，但调用成本上升、音乐质量未证明改善；按需 subagent 委派已落地，待真实 A/B 验收
...
7. 允许 Composer 按需把边界清晰的设计、实现或复核任务委派给具备最小只读工具集的 subagent，并通过真实 A/B 验证收益。

现有实现仍复用 Composer、`inspect_alda_source`、候选检查点、工作乐谱和 MIDI/WAV 链路，不引入分支乐谱、
通用工作流引擎或自动审美评分。subagent 只接收显式任务与必要上下文，可查询文档、检查片段和只读检查现有
乐谱，返回内容由 Composer 整合；不固定角色、段落家族或调用次数，也不把它扩展为通用 Multi-Agent 平台。
...
字段提交时连续恢复两次。当前要同时补齐游标安全和协议容错，并通过按需委派 A/B 验证作曲设计与 DSL
实现的职责隔离是否有效。
...
第二次运行证明这些字段不足以成为完整的作曲规格：计划曾从 420 秒压缩到 300 秒，又跟随 406 秒、291 秒的
实际结果回填为 297.14 秒。精确动机、和声区域、织体、主导声部、前后过渡和主题变形仍需由 Composer 在
创作上下文或具体委派任务中表达；`form_plan` 只验证结构登记，不承担音乐实现判定。
...
第二次真实运行表明，Composer 在上述步骤中仍同时承担艺术设计、拍数核算、Alda 编码与协议恢复。现已提供
按需 `delegate(task, context?)`：Composer 自行选择适合隔离的设计、实现或复核任务；subagent 具有文档查询、
片段检查和项目乐谱只读检查能力，返回文本只作为 Composer 的上下文，最终源码仍由现有链路组装、检查和提交。验收方案见
[Subagent 委派 A/B 验收](workflow-artifacts-and-agent-roles.md)。
...
### 第五阶段：按需委派 A/B（能力已实现，待人工验收）

- Composer 按任务需要调用具备最小只读工具集的 subagent，不固定角色、段落分组和调用次数；
- 委派只传显式 `task/context`，结果作为文本返回，最终校验和提交仍由 Composer 与现有宿主完成；
- 对照不委派基线测量总模型调用、委派次数、协议恢复、DSL 错误和人工完整试听结果；
- 若无可测收益，调整提示或撤回入口，不增加调度、Artifact 或 Multi-Agent 平台。
...
- 不预设 Worker 类型、按乐器或段落家族拆分，也不让 subagent 直接修改或持久化项目；
- 不把内部按段生产变成用户逐段试听或审批；
