...
>
> 状态：第二次代表性长曲、段落组装原型和成功配对角色 A/B 已完成；待人工试听与异题复验，音乐质量收益未证明
...
现有正式实现仍复用单 Agent、`inspect_alda_source`、候选检查点、工作乐谱和 MIDI/WAV 链路，不引入分支乐谱、
通用工作流引擎或自动审美评分。独立实验入口已用原生 Alda 和薄 Harness 完成一次成功配对运行，但它仍与
正式流程隔离；下一阶段是完整人工 A/B 和不同曲式或拍号的异题复验，不把它扩展为通用 Multi-Agent 平台。

确定性组装层的首个运行证据见
[段落组装与时间线机制验证](../iter/section-assembly-mechanism/README.md)：两段、两声部和错位进入已经通过
真实 Alda 2.3.3 解析验证，短一拍的 Worker 片段也能被拒绝。角色 A/B 的失败演进与成功配对结果见
[Composer–Worker–Reviewer 真实 A/B](../iter/composer-worker-reviewer-ab/README.md)。
...
字段提交时连续恢复两次。游标检查已经由实验纵切证明可行；首轮角色 A/B 则暴露 Composer 绝对拍数契约不可
靠模型稳定满足。当前薄 Harness 已改为量化段落预算、Worker 只写原生 Alda sequence，并在一次真实运行中让
两臂都形成完整 WAV；职责隔离是否改善音乐质量仍需完整试听与异题复验。
...
实际结果回填为 297.14 秒。该问题不通过扩张 `CompositionSpec` / `SectionContract` 为富音乐 IR 解决。
Composer 用普通结构化字段向 Worker 表达动机、和声、织体和发展意图；宿主只量化段落预算并验证 Alda 事实，
音乐承诺是否实现仍由 Reviewer 与完整试听判断。
...
实验纵切与角色 A/B 已能报告并拒绝以下问题；若未来接入正式流程，这些边界不得丢失：
...
第二次真实运行表明，单 Agent 在上述步骤中仍同时承担艺术设计、拍数核算、Alda 编码与协议恢复。独立任务
A/B 已让 Composer 声明音乐计划和段落比例、Harness 量化只读预算、两个段落家族 Worker 编写原生 Alda
sequence、固定模板连接为普通 Alda，并由独立 Reviewer 只读复核。一次成功配对只证明路径可行；详细事实与
后续门槛见[实验记录](../iter/composer-worker-reviewer-ab/README.md)和
[最终设计](../iter/composer-worker-reviewer-ab/design.md)。
...
### 第五阶段：任务专用角色 A/B（工程路径已验证，待人工与异题验收）

- [x] 从 Composer 移除绝对拍数，由 Harness 根据 tempo、meter、乐句网格和段落权重量化只读预算；
- [x] 固定主 Agent、两个段落家族 Worker、原生 Alda 模板和只读 Reviewer；
- [x] Worker 只交付每段每声部的原生 Alda sequence，不增加富音乐 IR、Cue 或技法专用分析；
- [x] 与现有单 Agent 形成一组完整 Alda、MIDI、WAV，并记录调用、恢复、游标、事件和时长事实；
- [ ] 对两份完整 WAV 做人工 A/B，并用不同曲式或拍号的长篇任务复验；
...
- Reviewer 能引用具体段落和最终 Alda 证据说明音乐计划是否实现，且该判断不伪装成机械证明；
...
- 不建设 Alda 之上的片段语言、Timeline DSL、Cue 系统或对位/和声专用分析器；
- 不把内部按段生产变成用户逐段试听或审批；
