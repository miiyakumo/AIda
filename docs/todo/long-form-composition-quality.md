...
>
> 状态：最终方案已确定；第一阶段的 Marker 实际时间已实现，其余待实施与真实试听验收
...
该方案复用现有单 Agent、`inspect_alda_source`、候选检查点、工作乐谱和 MIDI/WAV 链路，不引入多 Agent、
分支乐谱、通用工作流引擎或自动审美评分。

## 当前缺口

当前系统已经能完成 Alda 解析、候选检查点、MIDI/WAV 渲染和非静音门禁，但一次五分钟真实创作仍用了
14 次模型调用和 13 次工具往返。模型长期被总时长误差牵引，主要靠扩大循环收敛；达到总时长后才发现
声部尾部结构问题，并在过程中多次整体重写。

代码中的 Marker 检查会扫描源码，验证重复定义、未定义引用和先引用后定义；`ParseOutput` 现已读取
`alda parse` 返回的 `markers: { name: offset_ms }`，`ScoreInfo`、`inspect_score` 和
`inspect_alda_source` 都会按实际时间返回 Marker。事件时间线仍只聚合到全曲和声部，没有按 Marker 区间
切分；工作乐谱也只保存类型、说明和检查结果，没有与源码绑定的段落计划。

现有 `CandidateCheckpoint` 已保存候选源码和检查结果，但最终 `submit_result` 不能引用它，模型必须再次发送
完整 `alda_code`。现有修改也只能重新生成完整源码，没有基于已知 source hash 的受限增量操作。本案例的
最终源码只有 1,695 字节，未触及 32/64 KiB 上限，因此当前优先解决的是重复协议责任与全局重写，不是盲目
提高字节限制。
...
`play_score`、`render_score` 和 `/alda play work` 增加可选时间窗口。用户可以使用段落 ID，也可以直接说
...
| `src/alda.rs`、`src/audio.rs` | 支持按 Marker/秒数的 `--from` / `--to` 上下文窗口播放和导出 |
| `prompts/protocol.md`、渐进式 Skill | 固定长篇的结构先行、局部修正和试听边界 |
| `src/repl.rs`、控制面 | 展示段落表，并支持可选局部窗口播放/渲染 |
...
- [x] 输出 Marker 实际时间；
- [ ] 输出各段声部覆盖；
- 为缺失、乱序和明显偏离计划的 Marker 建立测试。

Marker 实际时间已经能替代段落位置手算；各段声部覆盖与计划边界校验仍需后续实现。
...
每一阶段都能独立验证；若 Marker 时间证据已经显著降低调用次数，后续仍按真实缺口推进，不提前扩展通用
Artifact、Role 或 Multi-Agent。
...
- 不要求所有声部等长或每段持续发声；
- 不因长篇任务引入 Director、Composer、Critic 等多 Agent；
