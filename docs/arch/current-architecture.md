...
按实际时间排序的 Marker、各声部结束时间与事件数，并把硬失败和时间线等诊断分开。Marker 时间直接取自
`alda parse` 的毫秒 offset；宿主拒绝负数或非有限 offset，但不要求 Marker 位于最后一个发声事件之前，
因为休止之后的 Marker 仍可合法存在。fragment 不套用整曲目标时长或项目配器约束，
