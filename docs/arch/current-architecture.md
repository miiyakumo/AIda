...
MIDI/WAV 并返回真实音频指标、以及真实发起播放。宿主工具往返和协议恢复不计作候选提交。SSE 按
tool-call index 分别聚合；若供应商仍
...
`inspect_alda_source` 始终可用，不依赖项目是否已有 work/current。它接受 `scope=fragment` 与最多 32 KiB
的短材料或完整临时源码，将源码写入系统临时文件后调用真实 `alda parse`，返回解析状态、总时长、各声部
结束时间与事件数，并把硬失败和时间线等诊断分开。临时文件离开调用即删除，不写入项目、不改变工作乐谱，
也不增加候选提交次数；fragment 检查不套用整曲目标时长或项目配器约束，固定工作流优先让模型一次检查
4–16 小节材料。候选校验失败时，返回模型的反馈进一步按“必须修正”“未检查或诊断”“已通过”分组；只有
硬失败是自动修正目标，时间线、事件空档、声部尾差和局部静音不会驱动模型强制铺满或对齐声部。
