...
工具调用前的模型文本在本轮仍完整展示，但回灌给下一次模型调用时只保留第一条非空行且最多 160 个
Unicode 字符，避免动作自述、旧计划和诊断猜测递归污染后续上下文。

模型请求在提供工具时显式发送 `parallel_tool_calls: false`，每次响应只允许调用一个工具。除 `submit_result`
外，模型始终可用 `delegate`、`lookup_alda_docs`、`inspect_alda_source` 和 `inspect_alda_patch`；项目会话另
提供 `inspect_score`、`render_score` 和 `play_score`。没有项目上下文时，补丁工具只允许引用本轮内存中的
`diagnostic` 失败基线；有项目上下文时还可引用 `work/current`。这些工具分别承担按需委派、读取固定官方
章节、检查临时源码或补丁、检查已有乐谱、生成 MIDI/WAV 并返回真实音频指标，以及真实发起播放。
...
交回 Composer。正常创作中 Composer 自行决定是否委派以及任务边界，宿主不预设 Worker、段落分组、依赖图或
Workflow；同类硬失败达到第三次时，宿主会强制切换为只诊断 Reviewer。所有 subagent 模型请求与 Composer
请求共同受模型调用上限约束，实际执行的内部工具计入工具往返，一次委派在 `delegations` 中记一次。每次实际
启动的委派都为 Composer 保留至少一次续写额度。
...
生成循环没有固定三次提交限制。宿主按硬失败类别维护当前诊断状态，包括连续次数、首次和最新失败源码哈希、
关键状态签名、当前假设、最近测试变化、文档查询与 Reviewer 状态。首次 `syntax_or_dsl` 失败必须先查询官方
章节；同类第二次失败后，下一步只能提交引用最新失败哈希的等价性声明并做局部复现，或委派独立诊断；第三次
进入 `diagnose_only`，除 `delegate` 外的动作会被拒绝。Reviewer 自动收到首次失败源码和最新失败源码，后者
不能覆盖前者。剩余调用进入最后 20% 时宿主注入 `wrap_up` 状态，只允许诊断、最小修复、最终验证或如实结束。
循环仍以成功、用户取消，或默认 15 分钟、24 次模型调用、8 次协议恢复为最终安全边界。运行时依赖已在应用
启动前门禁，因此缺少 Alda、FluidSynth 或 SoundFont 不会进入模型生成循环。

`inspect_alda_source` 始终可用，不依赖项目是否已有 work/current。它接受最多 32 KiB 源码，并区分
`scope=fragment|candidate`：两者都会通过调用期临时文件运行真实 Alda 静态检查，返回解析状态、总时长、
按实际时间排序的 Marker、相邻 Marker 划分的段落边界/事件/声部覆盖、各声部结束时间，并把硬失败和
时间线等诊断分开。Marker 时间直接取自 `alda parse` 的毫秒 offset；宿主拒绝负数或非有限 offset，但
不要求 Marker 位于最后一个发声事件之前，因为休止之后的 Marker 仍可合法存在。段内事件数按起音位置
唯一归属，跨段延音则按与每段相交后的合并发声时长计算覆盖率。fragment 不套用整曲目标时长或项目配器约束，
离开调用即删除且不作为恢复点，固定工作流优先用它检查 4–16 小节材料；candidate 套用当前项目的完整时长
和配器约束。无论检查成功、失败或超过源码检查大小，candidate 都留下带源码、哈希、检查结果、`form_plan`
和 `edit_scope` 的本轮内存检查点。失败检查点只能作为 `diagnostic` 补丁基线，不能通过 `candidate_ref` 提交；
补丁通过全部硬检查后才产生可提交检查点。candidate 预检仍不渲染、不写 work、不增加正式提交次数，也不会
强制模型下一步调用 `submit_result`；模型继续自行工作并负责最终正式提交。若随后命中模型调用、时间或协议
上限，或模型服务失败，应用把最新检查点保存为带校验哈希的 `revision.alda`，终端按有效性称为“完整候选
检查点”或“待修正诊断候选”，重启后的“继续修正”会重新载入它。候选校验失败时，返回模型的反馈进一步按
“必须修正”“未检查或诊断”“已通过”分组；只有硬失败是自动修正目标，时间线、事件空档、声部尾差和局部
静音不会驱动模型强制铺满或对齐声部。

目标时长下限不少于 120 秒的完整候选必须携带 4–10 段 `form_plan`，每段对应一个
`%section_<id>` Marker。宿主验证计划区间连续、Marker 精确对应并按序出现，实际边界容差为
`max(2 秒, 目标段时长 × 10%)`。计划随 pending revision、working score 和接受后的 version metadata
持久化，项目加载时重新验证；`/project accept` 还会根据当前 `work.alda` 重新解析并核对计划，避免源码与
元数据在保存后漂移。

段落事件哈希使用相对段首的起音时间、可读声部、发声时长和 Alda 返回的其余事件字段，因此前方段落变长
导致整段平移不会改变哈希。已有结构化基线时，新候选必须声明 `edit_scope`。`local` 只允许目标段变化，并
机械比较所有非目标段的计划语义、目标时长和事件哈希；`global` 明确表示整体重写。`inspect_score` 返回
所读工作稿或版本的 SHA-256 `source_hash`；`inspect_alda_patch` 对 `work/current/diagnostic` 的 SHA-256
基线应用 1–8 个唯一且互不重叠的文本替换，然后在临时文件中执行与完整候选相同的计划、范围和项目检查。
它不直接修改项目，检查通过后产生可由 `candidate_ref.source_hash` 引用的候选检查点。`diagnostic` 是本轮失败
候选的内存基线，不依赖 Project 上下文；因此一次性初次创作也能在失败后局部修复。补丁只允许命中当前最新
基线；存在更新的恢复或诊断候选时，拒绝从旧 work/current 派生补丁。

`play_score` 默认仍播放整曲；可选 `section_id` 时，根据实际 Marker 边界调用 Alda `--from/--to`，并将
`context_secs` 限制在前后 5–15 秒。终端与 JSONL 控制面提供同一能力：
`/alda play work|current section ID` 或 `alda_play.section_id`。局部播放只用于问题定位和修改前后比较，不是
逐段确认或接受版本的门禁；`render_score` 与候选自动渲染继续产生完整 WAV。
...
值。Alda 事件中的内部声部 ID 会通过 parse 结果的 alias 映射还原为用户别名；有别名时显示为
`别名（stock-instrument）`，否则依次使用可读 part name、stock instrument，内部指针只作为最后兜底。
同一映射同时用于临时源码的 parts 列表与“决定结尾”诊断。每个声部的首尾、事件数、实际发声时长、最大
空档、覆盖率，以及决定全曲结尾的声部、尾差和全局事件空档只作为诊断；晚进入、长休止和声部不等长本身
不失败。全局事件区间以 150 ms 容差合并，避免把正常量化间隙误判成长静音。
...
系统提示由精简核心协议、能力边界、固定工作流、项目偏好、可选 advisory Skill 和默认角色按固定顺序编译，
不再自动注入 `prompts/alda-reference.md`。工具 description 只携带与自身调用直接相关的局部纪律；DSL 事实
通过 `lookup_alda_docs` 按需读取。完整官方快照保存在 `vendor/alda-docs/2.4.3/`，含来源提交与 EPL-2.0
许可证；当前运行时兼容目标仍为 Alda 2.3.3，精简参考只作为维护和示例测试资料，不是每轮模型上下文。
