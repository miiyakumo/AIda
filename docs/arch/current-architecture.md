...
当前没有独立的创作计划、草稿或待接受候选生命周期。所有普通自然语言共享同一个 Agent 生成入口；模型
若只返回文字，界面将其视为等待补充信息；模型若提交与当前乐谱不同且通过必要检查的完整 Alda，应用会
立即保存为新版本并更新 `current.alda`。因此当前“通过客观校验的候选”和“有效版本”仍是同一个状态，
这是待处理的[渐进式创作与版本语义](../todo/iterative-creation-workflow.md)需求。
