...
和配器约束，并把最新源码及检查结果记为故障恢复检查点。candidate 预检仍不渲染、不写 work、不增加正式
提交次数，也不会强制模型下一步调用 `submit_result`；模型继续自行工作并负责最终正式提交。若随后命中模型
调用、时间或协议上限，或模型服务失败，应用把该检查点保存为带校验哈希的 `revision.alda`，终端明确称为
“完整候选检查点”，重启后的“继续修正”会重新载入它。候选校验失败时，返回模型的反馈进一步按“必须修正”
“未检查或诊断”“已通过”分组；只有硬失败是自动修正目标，时间线、事件空档、声部尾差和局部静音不会
驱动模型强制铺满或对齐声部。
...
所读工作稿或版本的 SHA-256 `source_hash`；`inspect_alda_patch` 对
work/current 的 SHA-256 基线应用 1–8 个唯一且互不重叠的文本替换，然后在临时文件中执行与完整候选相同的
计划、范围和项目检查。它不直接修改项目，检查通过后产生可由 `candidate_ref.source_hash` 引用的候选检查点。
补丁只允许命中当前最新基线（恢复候选优先于 work，work 优先于 current）；存在更新的恢复候选时，拒绝从
旧 work/current 派生补丁，模型仍可基于已注入上下文提交完整候选。
