...
To project and business constraints:

```
"How should I handle payment failures?"
    ↑ Ask: What are the business rules for retries?
    ↑ Check: project requirements (transaction requirements)
    ↑ Check: SLA (availability requirements)
```

| Question | Trace To | Ask |
|----------|----------|-----|
| Retry policy | Project requirements | What's acceptable latency for retry? |
| User experience | Product requirements | What message should users see? |
| Compliance | Audit requirements | What must be logged for audit? |
