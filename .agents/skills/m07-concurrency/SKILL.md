...
**CRITICAL**: Don't just fix the error. Trace UP to find project constraints.

### Context Detection Table

| Context Keywords | Inspect | Key Constraint |
|-----------------|---------|----------------|
| Web API, HTTP, axum, actix, handler | Framework and project state model | Handlers may run on any thread |
| 交易, 支付, trading, payment | `m09-domain` and `m13-domain-error` | Audit and thread safety |
| gRPC, kubernetes, microservice | Deployment and tracing configuration | Distributed tracing |
| CLI, terminal, clap | Command execution model | Usually single-threaded unless proven otherwise |

### Example: Web API + Rc Error

```
"Rc cannot be sent between threads" in Web API context
    ↑ DETECT: "Web API" → Inspect framework and project state
    ↑ FIND: shared application state must be thread-safe
    ↑ FIND: Rc cannot cross worker-thread boundaries
...
"Send not satisfied for my type"
    ↑ Ask: What project context and runtime constraints apply?
...
|-----------|----------|----------|
| Send/Sync in Web | Project code + m11-ecosystem | What's the framework state pattern? |
| Send/Sync in CLI | Project code | Is multi-threading really needed? |
